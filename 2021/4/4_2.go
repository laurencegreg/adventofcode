...
func calcBingo(bingoGrid [][]box, number string) int {
...
func main() {
	...
	var loosers [][][]box
	it := 0
	for len(bingoGrids) != 1 {
		fmt.Println("call for ", values[it])
		callNumber(bingoGrids, values[it])
		for _, bingoGrid := range bingoGrids {
			if calcBingo(bingoGrid, values[it]) == 0 {
				loosers = append(loosers, bingoGrid)
			}
		}
		it++
		bingoGrids = loosers
	...
	for res == 0 {
		fmt.Println("call for ", values[it])
		callNumber(bingoGrids, values[it])
		res = calcBingo(bingoGrids[0], values[it])
		it++
	}
